...
async function getLiveData({ force = false } = {}) {
  ...
  const run = (async () => {
    const cfg = getConfig();
    const { issues, allCards, unassignedCards } = await fetchLive(cfg);
    // allCards (full assigned set, incl. won/churn) is kept for the Jira Update
    // Check tab, unassignedCards for the wins tab; api/data strips both so the
    // main dashboard payload stays lean.
    const data = { generatedAt: new Date().toISOString(), count: issues.length, issues, allCards, unassignedCards };
