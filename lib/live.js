...
async function getLiveData({ force = false } = {}) {
  ...
  const run = (async () => {
    const cfg = getConfig();
    const { issues, allCards } = await fetchLive(cfg);
    // allCards (full assigned set, incl. won/churn) is kept for the Jira Update
    // Check tab; api/data strips it so the main dashboard payload stays lean.
    const data = { generatedAt: new Date().toISOString(), count: issues.length, issues, allCards };
