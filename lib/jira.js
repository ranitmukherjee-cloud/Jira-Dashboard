...
async function fetchLive(cfg, concurrency = 8) {
  ...
  const issues = raw
    ...
    .filter((i) => i.assignee && i.stageGroup !== 'won' && i.stageGroup !== 'churn' && i.status !== 'In Progress' && !EXCLUDED_CLOSED_STATUSES.includes(i.status));
  issues.sort((a, b) => new Date(b.updated) - new Date(a.updated));
  return { issues, total: raw.length, excluded: raw.length - issues.length, fullChangelogFetches: truncated.length };
}
