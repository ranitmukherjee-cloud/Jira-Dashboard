...
async function fetchLive(cfg, concurrency = 8) {
  ...
  const allCards = transformed.slice().sort((a, b) => new Date(b.updated) - new Date(a.updated));
  return { issues, allCards, total: raw.length, excluded: raw.length - issues.length, fullChangelogFetches: truncated.length };
}
