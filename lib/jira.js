...
const CHURN_STATUSES = ['Churn', 'Churn/Check in 3 Months'];
const CHECK_IN_STATUS = 'Check in 3 Months';

// Everything the consolidated Overview needs in one pull: active pipeline,
// won/closed, check-in-3-months and churn — spanning sets the main dashboard
// deliberately hides (won/churn), so it can't be built from /api/data alone.
const OVERVIEW_STATUSES = [
  ...new Set([...ACTIVE_PIPELINE_STATUSES, ...WON_STATUSES, CHECK_IN_STATUS, ...CHURN_STATUSES]),
];
...
// Posts an ADF-bodied comment (used by the Fireflies -> Jira sync).
async function addComment(cfg, issueKey, adfBody) {
  const res = await fetch(`https://${cfg.domain}/rest/api/3/issue/${issueKey}/comment`, {
    method: 'POST',
    headers: {
  ...
  if (!res.ok) throw new Error(`Jira add-comment error ${res.status} for ${issueKey}: ${await res.text()}`);
  return res.json();
}

// Uploads a file as an issue attachment (used for the Fireflies transcript,
// so it shows up in the Attachments panel instead of cluttering the comment
// feed). Returns { id, filename, url } — url is the attachment's authenticated
// download link, safe to drop into a comment as a "doc link".
async function addAttachment(cfg, issueKey, filename, buffer, mimeType) {
  const form = new FormData();
  form.append('file', new Blob([buffer], { type: mimeType }), filename);
  const res = await fetch(`https://${cfg.domain}/rest/api/3/issue/${issueKey}/attachments`, {
    method: 'POST',
    headers: {
      Authorization: authHeader(cfg),
  ...
  if (!res.ok) throw new Error(`Jira add-attachment error ${res.status} for ${issueKey}: ${await res.text()}`);
  const data = await res.json();
  const meta = Array.isArray(data) ? data[0] : data;
  return { id: meta?.id, filename: meta?.filename, url: meta?.content };
}

// Cheap idempotency check for webhook retries: does any existing comment on
// this issue already contain `needle` (we embed the Fireflies meeting id).
async function hasCommentContaining(cfg, issueKey, needle) {
  const res = await fetch(
    `https://${cfg.domain}/rest/api/3/issue/${issueKey}/comment?maxResults=100&orderBy=-created`,
    { headers: { Authorization: authHeader(cfg), Accept: 'application/json' } }
  );
  if (!res.ok) throw new Error(`Jira get-comments error ${res.status} for ${issueKey}: ${await res.text()}`);
  const data = await res.json();
  return (data.comments || []).some((c) => JSON.stringify(c.body).includes(needle));
}
